...
export default function TasksPage() {
  ...
  const openEdit = (task: any) => {
    ...
    setChecklistItems(task.checklists?.map((c: any) => c.text) || [])
    setShowCreate(true)
  }
    ...
    try {
      if (editTask) {
        await fetch("/api/tasks", {
          ...
          body: JSON.stringify({ id: editTask.id, title: title.trim(), description, priority, date, dueDate }),
        })
      } else {
        await fetch("/api/tasks", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ title: title.trim(), description, priority, date, dueDate, checklists: checklistItems }),
        })
      }
      setShowCreate(false)
      resetForm()
  ...
  const addChecklistItem = () => {
    ...
    setChecklistItems([...checklistItems, checklistText.trim()])
    setChecklistText("")
  }
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">تابع مهامك وإنجازاتك اليومية</p>
          </div>
          <button
            onClick={openCreate}
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          />
          {dateFilter && (
            <button onClick={() => setDateFilter("")} className="text-xs text-red-500 hover:underline">إلغاء الفلتر</button>
          )}
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              ...
              placeholder="بحث..."
              className="w-full h-9 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 pr-9 pl-3 text-xs text-gray-900 dark:text-white placeholder:text-gray-400"
            />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border-2 border-dashed border-gray-300 dark:border-gray-700 p-16 text-center">
            ...
            <button onClick={openCreate} className="rounded-xl bg-purple-600 px-6 py-2.5 text-sm font-medium text-white hover:bg-purple-700">
              إضافة مهمة
            </button>
          </div>
        ) : (
                  ...
                  <div className="flex items-center gap-3 p-4">
                    <button onClick={() => toggleStatus(task)} className="shrink-0">
                      {task.status === "completed" ? (
                        ...
                        <Circle className="h-6 w-6 text-gray-300 dark:text-gray-600 hover:text-purple-400 transition-colors" />
                      )}
                    </button>
                    ...
                    <div className="flex-1 min-w-0 cursor-pointer" onClick={() => toggleStatus(task)}>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded", p?.color)}>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button onClick={() => openEdit(task)} className="p-1.5 rounded-lg text-gray-400 hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-950/20 transition-all">
                        <Edit3 className="h-4 w-4" />
                      </button>
                      <button onClick={() => deleteTask(task.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/20 transition-all">
        ...
        {tasks.length > 0 && (
          <div className="text-center text-xs text-gray-400">
            {tasks.filter((t) => t.status === "completed").length}/{tasks.length} مهام مكتملة
            {" · "}
            {tasks.filter((t) => t.priority === "critical" && t.status !== "completed").length} مهمة حرجة متبقية
          </div>
        )}
      </motion.div>
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-bold text-gray-900 dark:text-white">
                  {editTask ? "تعديل مهمة" : "مهمة جديدة"}
                </h2>
              ...
              <div className="space-y-4">
                {error && (
                  <div className="flex items-center gap-2 p-3 rounded-lg bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 text-sm text-red-600">
                ...
                <div>
                  ...
                  <input value={title} onChange={(e) => setTitle(e.target.value)}
                    placeholder="مثلاً: مراجعة حملة ميتا"
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mb-2">
                      ...
                      <button onClick={addChecklistItem}
                        className="px-3 rounded-lg bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 text-sm font-medium hover:bg-purple-200 dark:hover:bg-purple-900/50 transition-colors">
                        + إضافة
                      </button>
                    </div>
                    {checklistItems.map((item, i) => (
                      <div key={i} className="flex items-center gap-2 p-1.5 rounded-lg bg-gray-50 dark:bg-gray-800/30 mb-1">
                        ...
                        <button onClick={() => removeChecklistItem(i)} className="text-gray-400 hover:text-red-500">
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button onClick={() => { setShowCreate(false); resetForm() }}
                    className="px-4 rounded-xl border border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                    إلغاء
                  </button>
                </div>
              </div>
